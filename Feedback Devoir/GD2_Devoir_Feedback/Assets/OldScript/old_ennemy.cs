...
public class old_ennemy : MonoBehaviour
{
    ...
    private void shoot()
    {
        ...
        if (Physics.Raycast(this.transform.position, this.transform.forward, out hit, range, layertohit))
        {
            ...
            if (isplayer)
            {
                obj.GetComponent<old_Player>().TakeDamage(dmg);//return if ennemy died from the shoot                       
            }

        }
    }
    public void TriggerhearPlayer(old_Player player) // hear the player, change the ennemy state // if player is in state "attacking" do nothing
    {
    ...
    public void Triggerseeplayer(old_Player player,bool enter)//change state of the ennemy if the player cross the trigger box of detection
    {
        if(enter)
        {
