...
[System.Serializable]
public enum old_movementspeeds// explicit
{
...
public class old_Player : MonoBehaviour
{
    ...
    public List<old_weapon> PlayerWeapons = new List<old_weapon>();//all weapon currently possesed by the player
    private List<GameObject> objweapon = new List<GameObject>();// all 3D object to of each weapon possesed by the player
    public old_weapon crtweapon; // current weapon equipt
    ...
    public old_movementspeeds speed = old_movementspeeds.nomove; // state of speed of the player
   ...
   // float JumpForce = 10;//
    ...
    public old_gdsoundmanager soundcheck; //object that manage the detection of ennemys
    ...
    private void Movement()//manage movemnt of the player
    {
        ...
        {
            speed = old_movementspeeds.nomove;
        }
        else 
        {
            if(crouch)
            {
                speed = old_movementspeeds.crouchspeed;
                noise = 0;
            }
            else if(run)
            {
                speed = old_movementspeeds.runspeed;
                noise = 15;
            }
            else
            {
                speed = old_movementspeeds.walkspeed;
                noise = 5;
        ...
        if(camera!= null)
        {
            ...
            if(Physics.Raycast(camera.transform.position, camera.transform.forward ,out hit, 15)) //cast ray from the center of the camera
            {
               GameObject obj = hit.collider.gameObject;
                ...
                {
                   bool dooropen = obj.GetComponent<old_door>().tryOpenDoor(keys);
                }            
            }
        }
    }
...
public class old_weapon 
{
    ...
    public old_weapon(string name, int loadercapacity, int currentammo, float frequency, int damage, int noise, float reloadtime, old_WeaponType type, float range)//constructpr
    {
        Name = name;
    ...
    public float shootWeapon(Camera cam,old_Player player,LayerMask layertohit)
    {
        ...
        if (Currentammo > 0 && lastshoot + (1 / Frequency) < Time.time)// shoot done if enough ammo, and cooldown is ok
        {
            ...
            if (cam != null)
            {
                ...
                if (Physics.Raycast(cam.transform.position, cam.transform.forward, out hit, Range, layertohit))
                {
                    ...
                    if (isennemy)
                    {
                        bool ennemydead = obj.GetComponent<old_ennemy>().TakeDamage(Damage);//return if ennemy died from the shoot
                        if(ennemydead)
                        {
